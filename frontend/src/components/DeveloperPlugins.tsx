  ...
  const renderWorkspace = () => (
    <section className="dashboard-developer-workspace">
      <div className="dashboard-panel dashboard-developer-list">
        ...
        <div className="dashboard-developer-plugin-list">{plugins.map((plugin) => <button className={`dashboard-developer-plugin-row${selectedPluginId === plugin.pluginId ? ' is-selected' : ''}`} type="button" key={plugin.pluginId} onClick={() => { setSelectedPluginId(plugin.pluginId); setVersionForm({ version: '0.2.0', manifest: JSON.stringify(plugin.versions[0]?.manifest ?? {}, null, 2), changelog: '', entryUrl: plugin.versions[0]?.entryUrl ?? '', entryCode: plugin.versions[0]?.entryCode ?? '', bundleFileName: '', bundleArchiveDataUrl: '', archiveFileName: plugin.versions[0]?.hasBundleArchive || plugin.versions[0]?.bundleArchiveDataUrl ? 'Using previous ZIP package' : '', hasBundleArchive: Boolean(plugin.versions[0]?.hasBundleArchive || plugin.versions[0]?.bundleArchiveDataUrl) }); }}><span className="dashboard-developer-plugin-mark"><Sparkles size={16} /></span><span><strong>{plugin.name}</strong><small>{plugin.pluginId} · v{plugin.versions[0]?.version ?? 'draft'}</small></span><em className={`dashboard-plugin-status is-${plugin.status}`}>{statusLabel(plugin.status)}</em></button>)}</div>
        )}
