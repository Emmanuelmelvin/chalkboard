...
export default function DeveloperPlugins() {
  ...
  const [versionForm, setVersionForm] = useState({ version: '0.2.0', manifest: '', changelog: '', entryUrl: '', entryCode: '', bundleFileName: '', bundleArchiveDataUrl: '', archiveFileName: '' });
  ...
  const loadPlugins = async () => {
    ...
    try {
      ...
      if (nextPlugin) setVersionForm({ version: '0.2.0', manifest: JSON.stringify(nextPlugin.versions[0]?.manifest ?? {}, null, 2), changelog: '', entryUrl: nextPlugin.versions[0]?.entryUrl ?? '', entryCode: nextPlugin.versions[0]?.entryCode ?? '', bundleFileName: '', bundleArchiveDataUrl: nextPlugin.versions[0]?.bundleArchiveDataUrl ?? '', archiveFileName: nextPlugin.versions[0]?.bundleArchiveDataUrl ? 'Using previous ZIP package' : '' });
    } catch (loadError) {
  ...
  const handleCreate = async (event: FormEvent) => {
    ...
    try {
      ...
      setVersionForm({ version: '0.2.0', manifest: JSON.stringify(payload.plugin.versions[0]?.manifest ?? {}, null, 2), changelog: '', entryUrl: payload.plugin.versions[0]?.entryUrl ?? '', entryCode: payload.plugin.versions[0]?.entryCode ?? '', bundleFileName: '', bundleArchiveDataUrl: payload.plugin.versions[0]?.bundleArchiveDataUrl ?? '', archiveFileName: payload.plugin.versions[0]?.bundleArchiveDataUrl ? 'Using previous ZIP package' : '' });
      setCreateOpen(false);
  ...
  const handleCreateVersion = async (event: FormEvent) => {
    ...
    try {
      ...
      setVersionForm((current) => ({ ...current, version: '0.3.0', changelog: '', entryCode: '', bundleFileName: '', bundleArchiveDataUrl: '', archiveFileName: '' }));
  ...
  const handleArchiveFileChange = (event: ChangeEvent<HTMLInputElement>, target: 'create' | 'version') => {
    const file = event.target.files?.[0];
    if (!file) return;
    ...
    reader.onload = () => {
      ...
      else setVersionForm((current) => ({ ...current, bundleArchiveDataUrl: reader.result as string, archiveFileName: file.name }));
      setError('');
    };
  ...
  const renderCreateForm = () => (
    <section className="dashboard-developer-create-panel dashboard-developer-create-screen">
      ...
      <form className="dashboard-developer-form" onSubmit={handleCreate}>
        ...
        <div className="dashboard-developer-upload-grid"><div><label htmlFor="developer-plugin-manifest-file">Manifest file <small>optional if editing below</small></label><input id="developer-plugin-manifest-file" type="file" accept="application/json,.json" onChange={(event) => handleManifestFileChange(event, 'create')} /><span className="dashboard-developer-upload-note">{createForm.manifestFileName || 'Choose manifest.json'}</span></div><div><label htmlFor="developer-plugin-bundle">Plugin bundle <small>JavaScript · 480 KB max</small></label><input id="developer-plugin-bundle" type="file" accept="text/javascript,.js,.mjs" onChange={(event) => handleBundleFileChange(event, 'create')} /><span className="dashboard-developer-upload-note">{createForm.bundleFileName || 'Choose index.js'}</span></div><div><label htmlFor="developer-plugin-archive">ZIP package <small>2 MB max</small></label><input id="developer-plugin-archive" type="file" accept=".zip,application/zip,application/x-zip-compressed" onChange={(event) => handleArchiveFileChange(event, 'create')} /><span className="dashboard-developer-upload-note">{createForm.archiveFileName || 'Choose plugin.zip'}</span></div></div>
  ...
  const renderWorkspace = () => (
    <section className="dashboard-developer-workspace">
      <div className="dashboard-panel dashboard-developer-list">
        ...
        <div className="dashboard-developer-plugin-list">{plugins.map((plugin) => <button className={`dashboard-developer-plugin-row${selectedPluginId === plugin.pluginId ? ' is-selected' : ''}`} type="button" key={plugin.pluginId} onClick={() => { setSelectedPluginId(plugin.pluginId); setVersionForm({ version: '0.2.0', manifest: JSON.stringify(plugin.versions[0]?.manifest ?? {}, null, 2), changelog: '', entryUrl: plugin.versions[0]?.entryUrl ?? '', entryCode: plugin.versions[0]?.entryCode ?? '', bundleFileName: '', bundleArchiveDataUrl: plugin.versions[0]?.bundleArchiveDataUrl ?? '', archiveFileName: plugin.versions[0]?.bundleArchiveDataUrl ? 'Using previous ZIP package' : '' }); }}><span className="dashboard-developer-plugin-mark"><Sparkles size={16} /></span><span><strong>{plugin.name}</strong><small>{plugin.pluginId} · v{plugin.versions[0]?.version ?? 'draft'}</small></span><em className={`dashboard-plugin-status is-${plugin.status}`}>{statusLabel(plugin.status)}</em></button>)}</div>
        )}
