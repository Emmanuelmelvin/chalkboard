...
import { flushPendingCanvasSnapshots } from '@/services/canvasSnapshots';

export async function cleanupAbandonedRoomSnapshots() {
  const flushed = await flushPendingCanvasSnapshots();
  ...
  logger.info('Abandoned room canvas snapshots cleaned', { cleaned: cleaned.length, cutoff, flushed });
  return { cleaned: cleaned.length, flushed };
}
