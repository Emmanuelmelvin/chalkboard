...
import { Check, Clipboard, Code2, ExternalLink, FileCheck2, KeyRound, LoaderCircle, LogOut, ShieldCheck, Sparkles, UsersRound, WalletCards, XCircle } from 'lucide-react';
...
export default function AdminDashboard() {
  ...
  return (
    <div className="admin-app">
      ...
      <main className="admin-main"><header className="admin-header"><div><p className="admin-eyebrow">Chalkboard / {view === 'plugins' ? 'Plugin review' : view === 'billing' ? 'Plans & coupons' : 'Administrators'}</p><h1>{view === 'plugins' ? 'Review the next tool.' : view === 'billing' ? 'Shape access.' : 'Protect the people who run it.'}</h1></div><div className="admin-header-user"><span>{session?.user.platformRole === 'super_admin' ? 'Super Admin' : 'Admin'}</span><strong>{session?.user.displayName}</strong></div></header>
