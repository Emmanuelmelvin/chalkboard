...
import {
  ...
  useAdminsQuery,
  usePublishAdminPluginMutation,
  useRemoveAdminMutation,
  useRemoveAdminPluginMutation,
...
function AdminAvatar({
  ...
  if (!source || imageFailed) {
    return (
      <span
        className="admin-header-avatar admin-header-avatar-fallback"
        aria-label={name}
      >
        {initials}
      </span>
...
export default function AdminDashboard() {
  ...
  const [removeRegistryConfirmOpen, setRemoveRegistryConfirmOpen] =
    useState(false);
  ...
  const loadPlugins = async (filter = statusFilter) => {
    ...
    try {
      ...
      setSelectedPluginId((current) =>
        current && payload.plugins.some((plugin) => plugin.pluginId === current)
          ? current
          : (payload.plugins[0]?.pluginId ?? null),
      );
    } catch (loadError) {
      setError(
        loadError instanceof Error
          ? loadError.message
          : "We could not load plugin submissions.",
      );
    } finally {
      setBusy(false);
    }
    ...
    try {
      ...
      setError(
        verifyError instanceof Error
          ? verifyError.message
          : "That 2FA code was not accepted.",
      );
    } finally {
      setBusy(false);
    }
  ...
  const handleReview = async (
    ...
    if (
      decision === "approved" &&
      (!testResult ||
        testResult.pluginId !== selectedPlugin.pluginId ||
        !testResult.passed)
    ) {
    ...
    try {
      ...
      setError(
        reviewError instanceof Error
          ? reviewError.message
          : "We could not update this plugin review.",
      );
    } finally {
      setBusy(false);
    }
  ...
  const runPluginTest = () => {
    ...
    if (!manifest || manifest.id !== selectedPlugin.pluginId)
      issues.push("Manifest ID does not match the plugin ID.");
    if (version && manifest?.version !== version.version)
      issues.push("Manifest version does not match the submitted version.");
    if (!manifest || !Array.isArray(manifest.permissions))
      issues.push("Manifest permissions are missing.");
    if (
      !manifest ||
      !manifest.contributes ||
      typeof manifest.contributes !== "object"
    )
      issues.push("Manifest contributions are missing.");
    if (!version?.entryCode?.trim()) {
      ...
      try {
        ...
        issues.push(
          `Bundle syntax error: ${bundleError instanceof Error ? bundleError.message : "invalid JavaScript"}`,
        );
      }
    }
    ...
    setTestResult({
      ...
      passed,
      message: passed
        ? `Manifest, contribution, and JavaScript bundle passed.${version?.hasBundleArchive || version?.bundleArchiveDataUrl ? " ZIP package attached." : " No ZIP package attached."}`
        : issues.join(" "),
    });
    setError("");
    setNotice(
      passed ? "Plugin smoke test passed." : "Plugin smoke test found issues.",
    );
  };
    ...
    try {
      ...
      setError(
        publishError instanceof Error
          ...
          : "We could not publish this plugin.",
      );
    } finally {
      setBusy(false);
    }
    ...
    try {
      ...
      setError(
        removeError instanceof Error
          ? removeError.message
          : "We could not remove this plugin from the registry.",
      );
    } finally {
      setBusy(false);
    }
    ...
    try {
      ...
      setError(
        adminError instanceof Error
          ? adminError.message
          : "We could not load administrators.",
      );
    } finally {
      setBusy(false);
    }
  ...
  const handleAddAdmin = async (event: FormEvent) => {
    ...
    setNotice("");
    try {
      const payload = await addAdminMutation.mutateAsync({
        email: newAdminEmail,
        role: newAdminRole,
      });
      setAdmins((current) => [
        ...current.filter((admin) => admin.id !== payload.admin.id),
        payload.admin,
      ]);
      setNewAdminEmail("");
      ...
      setError(
        adminError instanceof Error
          ? adminError.message
          : "We could not add that administrator.",
      );
    } finally {
      setBusy(false);
    }
  ...
  const handleRemoveAdmin = async (admin: AdminUser) => {
    ...
    try {
      ...
      setError(
        adminError instanceof Error
          ? adminError.message
          : "We could not remove that administrator.",
      );
    } finally {
      setBusy(false);
    }
  ...
  if (twoFactorMode === "forbidden")
    return (
      <main className="admin-loading admin-loading-error">
        <ShieldCheck size={26} />
        <h1>Admin access required</h1>
        <p>
          {error || "This account is not allowed to use the admin console."}
        </p>
        <a href="/">Return to Chalkboard</a>
      </main>
    );
  if (twoFactorMode === "setup" && setup)
    return (
      <main className="admin-auth-shell">
        <section className="admin-auth-card">
          ...
          <p className="admin-eyebrow">First access / 2FA setup</p>
          <h1>Secure the console.</h1>
          <p className="admin-auth-copy">
            Add this account to your authenticator app. Admin actions stay
            locked until the code is verified.
          </p>
          <div className="admin-secret-block">
            <span>Setup key</span>
            ...
            >
              Copy key <Clipboard size={13} />
            </button>
          </div>
          <p className="admin-otpauth">
            If your authenticator supports it, use this provisioning URI:
          </p>
          <code className="admin-uri">{setup.otpauthUri}</code>
          <form className="admin-auth-form" onSubmit={handleVerify}>
    ...
    return (
      <main className="admin-auth-shell">
        <section className="admin-auth-card">
          ...
          <p className="admin-eyebrow">Admin verification</p>
          <h1>One more step.</h1>
          <p className="admin-auth-copy">
            Enter the six-digit code from your authenticator app to open the
            admin console.
          </p>
          <form className="admin-auth-form" onSubmit={handleVerify}>
            <label htmlFor="admin-login-code">Authenticator code</label>
            <input
              id="admin-login-code"
              value={code}
              onChange={(event) => setCode(event.target.value)}
            ...
            <button
              ...
              disabled={busy}
            >
              {busy ? "Checking…" : "Open admin console"}
            </button>
          </form>
          {error && <p className="admin-error">{error}</p>}
        </section>
  ...
  return (
    <div className="admin-app">
      <aside className="admin-sidebar">
        ...
        <p className="admin-sidebar-label">Control plane</p>
        <nav className="admin-nav" aria-label="Admin sections">
          <button
            className={view === "plugins" ? "is-active" : ""}
            type="button"
          ...
          <section className="admin-admin-workspace">
            <div className="admin-panel">
              <div className="admin-panel-heading">
                ...
                {session?.user.platformRole === "super_admin" && (
                  <span className="admin-status is-approved">Super Admin</span>
                )}
              </div>
              <div className="admin-admin-list">
                {admins.map((admin) => (
                  <div className="admin-admin-row" key={admin.id}>
                    ...
                    <em
                      className={`admin-status is-${admin.platformRole === "super_admin" ? "approved" : "in_review"}`}
                    >
              ...
              {session?.user.platformRole === "super_admin" && (
                <form className="admin-add-form" onSubmit={handleAddAdmin}>
                  <label htmlFor="admin-new-email">
                    Add an existing Chalkboard user
                  </label>
                  <div>
                    <input
                      id="admin-new-email"
                  ...
                  <small>
                    The user must sign in once before they can be promoted.
                  </small>
                </form>
              )}
              {(error || notice) && (
                <p
                  className={`admin-feedback${notice && !error ? " is-success" : ""}`}
                >
                  {error || notice}
                </p>
              )}
          ...
          <>
            <section className="admin-stat-grid">
              ...
              <article>
                ...
                <strong>{selectedPlugin?.plan === "pro" ? "PRO" : "—"}</strong>
                <small>
                  {selectedPlugin ? selectedPlugin.name : "choose a plugin"}
                </small>
              </article>
              <article>
                ...
                <small>2FA verified session</small>
              </article>
            </section>
            <section className="admin-plugin-workspace">
              <div className="admin-panel admin-plugin-list">
                <div className="admin-panel-heading">
                  <div>
                  ...
                  <select
                    ...
                    onChange={(event) => {
                      ...
                      void loadPlugins(event.target.value);
                    }}
                  >
                ...
                {busy && plugins.length === 0 ? (
                  <div className="admin-empty">
                    <LoaderCircle className="admin-spin" size={18} /> Loading
                    queue…
                  </div>
                ) : plugins.length === 0 ? (
                  <div className="admin-empty">
                    ...
                    <span>
                      New submissions will appear here when developers send them
                      for review.
                    </span>
                  </div>
                ) : (
                  <div className="admin-queue">
                    {plugins.map((plugin) => (
                      <button
                        className={`admin-queue-row${selectedPluginId === plugin.pluginId ? " is-selected" : ""}`}
                        type="button"
                        key={plugin.pluginId}
                        onClick={() => setSelectedPluginId(plugin.pluginId)}
                      >
                        <span className="admin-queue-icon">
                          {plugin.logoUrl || plugin.logoDataUrl ? (
                            <img
                              src={plugin.logoUrl || plugin.logoDataUrl || ""}
                              alt=""
                            />
                          ) : (
                            <Code2 size={15} />
                        ...
                        <span>
                          ...
                          <small>
                            {plugin.author?.email || "Unknown author"} · v
                            {plugin.versions[0]?.version || "—"}
                          </small>
                        </span>
                        <em className={`admin-status is-${plugin.status}`}>
              ...
              <div className="admin-panel admin-plugin-detail">
                {selectedPlugin ? (
                  <>
                    <div className="admin-detail-heading">
                      <div>
                        <p className="admin-eyebrow">Submission detail</p>
                        <h2>{selectedPlugin.name}</h2>
                        <span>{selectedPlugin.pluginId}</span>
                      </div>
                      <em
                        className={`admin-status is-${selectedPlugin.status}`}
                      >
                        {formatStatus(selectedPlugin.status)}
                      </em>
                    </div>
                    <div className="admin-detail-meta">
                      <span>
                        Author{" "}
                        <strong>
                          {selectedPlugin.author?.displayName || "Unknown"}
                        </strong>
                      </span>
                      <span>
                        Access{" "}
                        <strong>
                          {selectedPlugin.plan === "pro" ? "Pro" : "Free"}
                        </strong>
                      </span>
                      <span>
                        Updated{" "}
                        <strong>{formatDate(selectedPlugin.updatedAt)}</strong>
                      </span>
                    </div>
                    <p className="admin-detail-description">
                      {selectedPlugin.description}
                    </p>
                    <div className="admin-version-card">
                      <div>
                        <p className="admin-eyebrow">Latest version</p>
                        <strong>
                          v{selectedPlugin.versions[0]?.version || "—"}
                        </strong>
                        <small>
                          {selectedPlugin.versions[0]?.changelog ||
                            "No changelog provided."}
                        </small>
                      </div>
                      {selectedPlugin.versions[0]?.entryUrl && (
                        <a
                          href={selectedPlugin.versions[0].entryUrl}
                          target="_blank"
                          rel="noreferrer"
                        >
                          Open bundle <ExternalLink size={13} />
                        </a>
                      )}
                    </div>
                    <details className="admin-manifest-details">
                      <summary>Inspect manifest</summary>
                      <pre>
                        {JSON.stringify(
                          selectedPlugin.versions[0]?.manifest || {},
                          null,
                          2,
                        )}
                      </pre>
                    ...
                    <details className="admin-manifest-details">
                      <summary>Inspect uploaded bundle</summary>
                      <pre>
                        {selectedPlugin.versions[0]?.entryCode
                          ? `${selectedPlugin.versions[0].entryCode.slice(0, 12000)}${selectedPlugin.versions[0].entryCode.length > 12000 ? "\n\n/* preview truncated */" : ""}`
                          : "No inline bundle uploaded."}
                      </pre>
                    </details>
                    {["in_review", "approved"].includes(
                      ...
                      <div className="admin-review-form">
                        <label htmlFor="admin-review-notes">Review notes</label>
                        <textarea
                          id="admin-review-notes"
                          rows={3}
                          value={notes}
                          onChange={(event) => setNotes(event.target.value)}
                          placeholder="Record what was tested or what needs to change."
                        />
                        <div className="admin-review-actions">
                          {selectedPlugin.status === "in_review" && (
                            <>
                              <button
                                className="admin-secondary-button"
                                type="button"
                              ...
                              <button
                                className="admin-secondary-button admin-approve-button"
                                type="button"
                                disabled={busy}
                                onClick={() => {
                          ...
                          {selectedPlugin.status === "approved" && (
                            ...
                            >
                              <Sparkles size={14} /> Publish to catalogue
                            </button>
                          )}
                        </div>
                      </div>
                    )}
                    {selectedPlugin.status === "published" && (
                      <div className="admin-review-form">
                        <label>Registry controls</label>
                        <small>
                          Removing a plugin stops it from being returned to
                          boards and keeps its submission history for
                          administrators.
                        </small>
                        <div className="admin-review-actions">
                          <button
                            ...
                            disabled={busy}
                            onClick={() => {
                              void handleRemoveFromRegistry();
                            }}
                          >
                            <XCircle size={14} /> Remove from registry
                          </button>
                        </div>
                      </div>
                    )}
                  ...
                  <div className="admin-empty admin-detail-empty">
                    ...
                    <p>
                      Choose a plugin from the queue to inspect its manifest and
      ...
      {view === "plugins" && selectedPlugin && (
        <section
          className={`admin-quick-test${testResult?.pluginId === selectedPlugin.pluginId && testResult.passed ? " is-passed" : ""}`}
        >
          <div>
            <p className="admin-eyebrow">Test lab</p>
            <strong>
              {testResult?.pluginId === selectedPlugin.pluginId
                ? testResult.message
                : "Run the smoke test before approval."}
            </strong>
          </div>
          <div className="admin-quick-test-actions">
            <button
              className="admin-secondary-button"
              type="button"
              disabled={busy}
            ...
            >
              <FlaskConical size={14} /> Open sandbox
            </button>
          </div>
        </section>
      )}
      {sandboxOpen && selectedPlugin && (
        <AdminPluginSandbox
          key={`${selectedPlugin.pluginId}-${selectedPlugin.versions[0]?.version || "draft"}`}
          plugin={selectedPlugin}
          onClose={() => setSandboxOpen(false)}
        />
      )}
      {removeRegistryConfirmOpen && selectedPlugin && (
        <ConfirmModal
      ...
      {recoveryCodes.length > 0 && (
        <div className="admin-recovery-overlay">
          <section className="admin-recovery-card">
            ...
            <p className="admin-eyebrow">Save these once</p>
            <h2>Recovery codes</h2>
            <p>
              Store these somewhere secure. Each code can be used once if you
