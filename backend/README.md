...
- Rate limiting is backed by Redis (atomic `INCR` + `PEXPIRE` in one Lua script), so counters are shared across every replica and survive deploys. If Redis is unreachable the limiter degrades to bounded per-process counters (`RATE_LIMIT_FALLBACK_TO_MEMORY=true`) or fails closed when that flag is disabled.
- A catch-all limiter covers every `/api` route, with tighter limiters on the credential and abuse-prone surfaces: Google sign-in, admin TOTP verification, room join / password reset, plugin uploads, and checkout. Socket events (`reaction:send`, `hand:raise`, `chat:send`, room join) are keyed on the authenticated user id, not the socket id, so reconnecting does not reset a quota.
- Client IPs are resolved from `X-Forwarded-For` by counting back `TRUSTED_PROXY_HOP_COUNT` hops from the right. **Set this to the number of reverse proxies in front of the process before going to production** — with `0` the header is ignored entirely (correct for a directly exposed app), and setting it too high lets clients spoof their address and bypass every IP-keyed limit.
- Responses carry `RateLimit-Limit`, `RateLimit-Remaining`, `RateLimit-Reset`, and a `Retry-After` header (floored at 1s) on 429s.
